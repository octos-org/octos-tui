...
fn push_turn_flow(
    ...
    // Live reasoning for the active turn, shown for the WHOLE turn — including the
    // tool-calling phase before any final reply streams — as one coherent `·`
    // block ABOVE the reply and activity. (Previously gated on `live_reply`
    // existing, so during tool calls the thinking fell through to the committed
    // per-continuation render, scattering it among the activity cards.)
    if let Some(live_reasoning) = app.live_reasoning.get(&session.id) {
        if !live_reasoning.text.trim().is_empty() {
            push_message_block(lines, palette, "reasoning", &live_reasoning.text, width);
        }
    }
...
mod tests {
    ...
    use crate::{
        ...
        model::{
            ...
            DiffPreviewHunk, DiffPreviewLine, LiveReasoning, ModelStatus, SessionModelCatalog,
            SessionRuntimeStatus, SessionView, TurnActivitySummary,
        },
        store::Store,
    ...
    #[test]
    fn live_tail_renders_live_reasoning_before_streaming_reply() {
        let mut app = active_turn_app("final answer");
        let session_id = app.sessions[0].id.clone();
        let turn_id = app.sessions[0]
            .live_reply
            .as_ref()
            .expect("active turn")
            .turn_id
            .clone();
        app.live_reasoning.insert(
            session_id,
            LiveReasoning {
                turn_id,
                text: "thinking live".into(),
            },
        );
        ...
        let rendered: Vec<String> = lines
            ...
            .collect();
        let reasoning_idx = rendered
            .iter()
            .position(|line| line.contains("· thinking live"))
            .expect("live reasoning block rendered");
        let reply_idx = rendered
            .iter()
            .position(|line| line.contains("• final answer"))
            .expect("live reply block rendered");
        assert!(
            reasoning_idx < reply_idx,
            "live reasoning must render before the streaming reply: {rendered:?}"
        );
    }
